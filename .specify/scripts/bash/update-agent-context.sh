...
get_project_structure() {
    ...
    if [[ "$project_type" == *"plugin"* ]] || [[ "$project_type" == *"OpenCode"* ]]; then
        echo ".opencode/\\n.specify/\\nspecs/"
...
get_language_conventions() {
    ...
    if [[ "$lang" == *"TypeScript"* ]]; then
        echo "$lang: ESM modules, schema-validated boundaries, repository-first workflow orchestration\\n- Branch naming: feat-short-name, fix-short-name, refactor-short-name, init-short-name, test-short-name"
    else
